...
#ahh thats easy math why are you making me code it
love %>% 
  arrange(outcome)
...
# is there a way to split this by age so i can do it in one command?
# love %>%
  # select(gender,age) %>% 
  # summarise(avgAge = median(age))
...
# is there a way to assign variables and make it so it dose the math for me?
love %>% 
  filter(series == 2016) %>% 
  count(series,first_arrive)
# 11/25= .423
love %>% 
  filter(series == 2017) %>% 
  count(series,first_arrive)
...
# Yes, but not super significantly each day later joined reduces your chance of winning by -0.00722
...
love %>%
  ...
  lm(win ~ profession == "model", data = .) %>%
  broom::tidy()

# YOUR INTERPRETATION HERE:
#Yes but once again not a lot the estimator is -.00642 there is a high p value though
